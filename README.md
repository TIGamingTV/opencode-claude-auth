...
| `OPENCODE_CLAUDE_AUTH_REFRESH_MAX_COOLDOWN_MS` | Hard ceiling the escalating cooldown above can reach. Kept above the 5-minute proactive-refresh interval so sustained rate-limiting is actually throttled instead of re-hitting the token endpoint on every tick.                                                                 | `1800000` (30 min)                                                 |
