...
| `OPENCODE_CLAUDE_AUTH_REFRESH_COOLDOWN_MS`     | Base per-account cooldown after a rate-limited refresh, before the plugin retries the token endpoint. Escalates with consecutive failures and is jittered.                                                                                                                        | `15000`                                                            |
| `OPENCODE_CLAUDE_AUTH_REFRESH_MAX_COOLDOWN_MS` | Hard ceiling the escalating cooldown above can reach. Also gates the reactive path (a real request whose token is within 60s of expiry), so raising this trades request volume for how long a single-instance account can be refused a refresh attempt — keep it short.           | `60000`                                                            |
